    ...
    return (
        <>
            <style>{`
                ...
                .navegador-inner { max-width:1280px; margin:0 auto; padding:0 1.5rem; display:flex; align-items:center; height:64px; gap:2rem; }
                .navegador-brand { display:flex; align-items:center; gap:10px; text-decoration:none; flex-shrink:0; }
                .navegador-brand-icon { width:32px; height:32px; background:var(--green-light); border-radius:50%; display:flex; align-items:center; justify-content:center; }
                .navegador-brand-name { font-family:'DM Serif Display',serif; font-size:20px; color:#fff; }
                .navegador-links { display:flex; align-items:center; gap:0.25rem; flex:1; }
                @media(max-width:768px) { .navegador-links { display:none; } }
                .navegador-link { display:flex; align-items:center; gap:7px; padding:0.5rem 0.9rem; border-radius:8px; font-size:14px; color:rgba(255,255,255,0.65); text-decoration:none; transition:background 0.15s, color 0.15s; white-space:nowrap; }
                .navegador-link:hover { background:rgba(255,255,255,0.08); color:#fff; }
                .navegador-link.active { background:rgba(82,183,136,0.2); color:#fff; }
                .navegador-link svg { flex-shrink:0; }
                .navegador-right { display:flex; align-items:center; gap:1rem; margin-left:auto; }
            ...
            <div className="app-navegador">
                <div className="navegador-inner">
                    ...
                    <nav className="navegador-links">
                        {navLinks.map(link => (
                            <Link
                                key={link.label}
                                href={link.href}
                                className={`navegador-link${currentPath === new URL(link.href, window.location.origin).pathname ? ' active' : ''}`}
                            >
